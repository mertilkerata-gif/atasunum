...
export default function AIRecommendationsPage() {
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="p-6 space-y-4 max-w-[860px]">
        {loading ? (
          ...
          recs.map((rec, i) => {
            ...
            return (
              <div key={rec.id} className="rounded-[14px] border animate-fade-in"
                style={{ animationDelay: `${i * 40}ms`, background: 'var(--bg-surface)', borderColor: 'var(--border-faint)' }}>
                ...
                <div className="flex items-start gap-4 px-5 pt-4 pb-3"
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="ml-auto flex items-center gap-1 text-[9px] num" style={{ color: 'var(--text-ghost)' }}>
                        <Clock size={9} />
                        {new Date(rec.created_at).toLocaleTimeString('tr-TR', { hour: '2-digit', minute: '2-digit' })}
                      </span>
                    </div>
                    <h3 className="text-[13px] font-semibold" style={{ color: 'var(--text-primary)', letterSpacing: '-0.02em' }}>
                      {rec.summary}
                    </h3>
                    <p className="text-[11px] mt-1" style={{ color: 'var(--text-muted)' }}>{rec.risk_explanation}</p>
                  </div>
                </div>

                {/* Actions */}
                <div className="px-5 py-3 space-y-2">
                  <div className="text-[9px] uppercase tracking-[0.15em] mb-2" style={{ color: 'var(--text-ghost)' }}>
                    ...
                    return (
                      <div key={action.id}
                        ...
                        style={{
                          ...
                          border: `1px solid ${action.applied ? 'var(--border-hair)' : pc.border}`,
                        }}>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="text-[11.5px] font-medium leading-snug"
                            style={{ color: action.applied ? 'var(--text-ghost)' : 'var(--text-primary)', textDecoration: action.applied ? 'line-through' : 'none' }}>
                            {action.action_text}
                          </div>
                        ...
                        {action.station && (
                          <span className="text-[9px] px-1.5 py-px rounded shrink-0"
                            style={{ background: 'rgba(255,255,255,0.04)', color: 'var(--text-ghost)', border: '1px solid var(--border-hair)' }}>
                            {action.station}
                          </span>
                        )}
                        {action.applied
