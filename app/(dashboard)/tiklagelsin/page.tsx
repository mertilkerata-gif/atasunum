...
import { ShoppingCart, Plus, Minus, ChevronRight, Clock, CheckCircle, Truck, Store, ArrowLeft, Zap } from 'lucide-react'
...
export default function TiklaGelsinPage() {
  ...
  useEffect(() => {
    ...
    const delays: Partial<Record<OrderEventType, number>> = {
      ...
      PREPARATION_COMPLETED: 3000, PACKING_STARTED: 3500, READY: trackingOrder.channel === 'DELIVERY' ? 4000 : 2500,
      COURIER_ARRIVED: 3000, PICKED_UP: 2500,
    }
    const delay = delays[trackingOrder.status] ?? 3000
    const t = setTimeout(() => {
      const now = new Date().toISOString()
  ...
  if (view === 'kitchen') return (
    <KitchenView orders={orders} onAdvance={advanceOrder} onBack={() => setView('menu')}
      restaurantName={restaurant.name} pulse={pulse.score} pulseConfig={pulseConfig} />
  )
  if (view === 'tracking' && trackingOrder) return (
    <TrackingView order={trackingOrder} onNewOrder={() => { setTrackingOrder(null); setView('menu') }} onKitchen={() => setView('kitchen')} />
  )
  if (view === 'checkout') return (
    <CheckoutView cart={cart} total={cartTotal} channel={channel} restaurant={restaurant}
      customerName={customerName} setCustomerName={setCustomerName}
      address={address} setAddress={setAddress}
      onBack={() => setView('menu')} onPlace={placeOrder} setChannel={setChannel} />
  )
      ...
      <div className="p-6 grid grid-cols-12 gap-6">
        ...
        <div className="col-span-4 space-y-4">
          <button onClick={() => setView('kitchen')}
            className="w-full flex items-center justify-between bg-indigo-500/10 border border-indigo-500/30 rounded-xl px-4 py-3 hover:bg-indigo-500/15 transition-colors">
            <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1.5">
              {orders.filter(o => !['COMPLETED','CANCELLED'].includes(o.status)).length > 0 && (
                <span className="text-xs bg-indigo-500 text-white rounded-full w-5 h-5 flex items-center justify-center font-bold">
...
interface CheckoutViewProps {
  ...
  onBack: () => void
  onPlace: () => void
  setChannel: (v: 'DELIVERY' | 'PICKUP') => void
}
              ...
              return (
                <div key={step} className="flex items-center gap-3">
                  <div className={cn('w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 transition-all',
                    done ? 'border-emerald-500 bg-emerald-500' : active ? 'border-orange-500 bg-orange-500/20' : 'border-white/10')}>
...
interface KitchenViewProps {
  orders: LiveOrder[]
  onAdvance: (orderId: string, newStatus: OrderEventType) => void
  onBack: () => void
  restaurantName: string
  pulse: number
  pulseConfig: ReturnType<typeof getRiskConfig>
}

function KitchenView({ orders, onAdvance, onBack, restaurantName, pulse, pulseConfig }: KitchenViewProps) {
